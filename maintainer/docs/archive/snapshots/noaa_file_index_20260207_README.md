...
- `poetry run noaa-spec file-list --start-year 1975 --end-year 2025 --sleep-seconds 0.5 --retries 3 --backoff-base 0.5 --backoff-max 8`
...
- `poetry run noaa-spec location-ids --start-year 1975 --end-year 2025 --sleep-seconds 0.5 --retries 3 --backoff-base 0.5 --backoff-max 8`
...
- Single-file cleaning:
  `poetry run noaa-spec clean-parquet output/<station_id>/LocationData_Raw.parquet`
- Batch cleaning orchestration:
  `poetry run noaa-spec cleaning-run --mode <mode> --input-root <root> --input-format parquet`
